...
pub unsafe extern "C" fn region_force(script: *mut bw::AiScript) {
    let mut read = ScriptData::new(script);
    let comparison = read.read_u8();
    let amount = read.read_u16();
    let force_type = read.read_u8();
    let category = read.read_u8();
    let location = read.read_position();
    let dest = read.read_jump_pos();
    ...
    let region_id = match bw::get_region(location.center) {
        Some(r) => r,
        None => return,
    };
    ...
    let player = (*script).player;
    let ai_regions = bw::ai_regions(player);
    let region = ai_regions.offset(region_id as isize);

    #[repr(C)]
    struct AiRegionForce {
        state: u8,
        flags: u8,
        target_region_id: u16,
        center: bw::Point,
        needed_ground_strength: u16,
        needed_air_strength: u16,
        enemy_ground_strength: u16,
        enemy_air_strength: u16,
        own_ground_strength: u16,
        own_air_strength: u16,
    }
    let region = region as *mut AiRegionForce;

    let (ground, air) = match category {
        0 => ((*region).needed_ground_strength, (*region).needed_air_strength),
        1 => (
            (*region).own_ground_strength.saturating_add((*region).enemy_ground_strength),
            (*region).own_air_strength.saturating_add((*region).enemy_air_strength),
        ),
        2 => ((*region).enemy_ground_strength, (*region).enemy_air_strength),
        3 => ((*region).own_ground_strength, (*region).own_air_strength),
        _ => {
            bw_print!("region_force: Unknown category {}", category);
            return;
        }
    };
    ...
    let value = match force_type {
        ...
        1 => air,
        _ => {
            bw_print!("region_force: Unknown type {}", force_type);
            return;
        }
    };
    ...
    let condition_met = match comparison {
        ...
        10 => value == amount,
        _ => {
             bw_print!("region_force: Unknown comparison {}", comparison);
             false
        }
    };
